...
import {getAllPaginated, getUsersByRoleName, setCurrent, setSize, setType} from '../../../store/slices/getUsers.slice';
...
const GetUsersPanel = () => {
    ...
    const getByRoleQuery = () => {
        dispatch(getUsersByRoleName({role: userType, page: currentPage, pageSize: pageSize}));
    };

    useEffect(() => {
        userType ? getByRoleQuery() : getPaginatedUsers();
    ...
    return (
        <div className={css.get_users_panel}>
            ...
            <div className={css.users_block}>
                ...
                {!!users.length && users.map(user => <UserGeneralBlock key={user.id} user={user}/>)}
            </div>
        </div>
    );
};
